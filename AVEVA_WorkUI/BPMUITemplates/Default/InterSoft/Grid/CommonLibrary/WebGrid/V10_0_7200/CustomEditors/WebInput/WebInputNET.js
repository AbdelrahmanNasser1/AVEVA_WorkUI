/*
WebInput.NET CustomEditor V1.2 for WebGrid.NET Enterprise Version 5.0
...
- Support SmartWebResources and Elegant Style introduced in WebGrid 5
- Support dynamic default style introduced in WebGrid 7

Dependencies:
-Shared\ISCore.js (WebUI.NET Client Framework)
-WebInput\*.js (WebInput Core Scripts)
*/
...
function WebInputNET_Editor()
{
    ...
    this._Initialize = function()
    {
        ...
        if (!textEditor._IsInitialized)
        {
            ...
            if (typeof (wgEditCtlKBHandler) == "function")
                Listener.Append(0, htmlObj, "onkeydown", function()
                {
                    wgEditCtlKBHandler(grid, "WebInputNET");
                    WIOnKeyDown_v1();
                    WebInputNET_Keydown();
                });
                ...
                Listener.Append(0, htmlObj, "onkeydown", function()
                {
                    grid.EditKeyboardHandler("WebInputNET");
                    WIOnKeyDown_v1();
                    WebInputNET_Keydown();
                });

            Listener.Append(0, htmlObj, "onselectstart", function()
            {
            ...
            if (textEditor.UseDefaultStyle)
            {
                if (grid.IsElegantDefaultStyle())
                {
                    var fs = textEditor.FrameObj.style;
                    var ts = textEditor.HtmlObj.style;

                    grid.ApplyDefaultStyleBorder(fs);
                    fs.paddingLeft = "1px";
                    fs.paddingTop = "0px";
            ...
            ev.OnDirty += "WebInputNET_OnDirty('" + grid.Name + "', '" + textEditor.Id + "')";
    ...
    this.OnRefresh = function(controlID)
    {
        var textEditor = ISGetObject(controlID);
        var grid = this.CustomEditor.Grid;
        var wi = textEditor.FrameObj;

        if (!textEditor._IsInitialized)
            return;

        document.body.insertAdjacentElement("beforeEnd", wi);
        ...
        if (textEditor.DisplayFormat)
        {
            ...
            if (_ctl)
                document.body.insertAdjacentElement("beforeEnd", _ctl);
        }
        ...
        textEditor._IsInitialized = false;
    }
    ...
    this.OnEnterEditMode = function(activeCell, columnXml, isDirty)
    {
        ...
        wgDispObjGeneric(this.Grid, textEditorFrame, activeCell.element, 0, 1, false, "WebInputNET");
        ...
        if (wgi.dataType == "datetime")
        {
            ...
            {
                if (wgi.dataFormat.length == 1)
                    wgi.dataFormat = GetFullFormatDate(dti, wgi.dataFormat);
            }
        }

        textEditor._WGI = wgi;

        this.Grid.IsOnBlur = true;
        textEditorFrame.setActive();
        textEditor.DoFocus();
        textEditor.isDirty = false;
        textEditor.HtmlObj.isDirty = false;
        window.setTimeout(function() { textEditor.SetFocus(); }, 10); // #626
        this.Grid.IsOnBlur = false;
    ...
    this.OnSetValue = function(text, activeCell)
    {
        ...
        if (df != null || ef != null || dyf != null)
        {
            ...
            if (wgi.dataType == "datetime")
            {
                ...
                if (wgf.indexOf("h") < 0 && wgf.indexOf("H") < 0)
                    dd.Hour24 = dd.Hour12 = 1;
                ...
                if ((wgf.indexOf("h") < 0 && wgf.indexOf("H") < 0) && wgf.indexOf("m") < 0 && wgf.indexOf("s") < 0)
                {
                    with (cd)
                    {
                        // #942: if data format is already specified from Grid, do no need to try assigning CommonDateFormat.
                        // Otherwise, correct input format will become reversed in such dd/MMM/yyyy case
                        if (wgi.dataFormat == "")
                        {
                ...
                else
                {
                    ...
                    with (cd)
                    {
                        ...
                        var keyWord = IS.ie ? "UTC" : "GMT";
                        OldValue = NewValue = v.substr(0, v.indexOf(keyWord) - 1);
                    }
                }
            ...
            else
            {
                ...
                else
                {
                    ...
                    {
                        textEditor.Text = textEditor.EditText = textEditor.Value = textEditor.HtmlObj.value
							= textEditor.EditFormat.ParseRawText(text);
                    }
                }
            }
        }
    ...
    this.GetText = function()
    {
        ...
        if (df != null || ef != null || dyf != null)
        {
            ...
            if (wgi.dataType == "datetime")
            {
                ...
                if (textEditor.DateTimeEditor != null && textEditor.DateTimeEditor.IsEnabled)
                {
                    v = textEditor.GetDateTimeValueData().toString();
                }
                else
                {
                    var dd = cd.DateData;
                    ...
                    var d = new Date(dd.Year, dd.Month, parseFloat(dd.Day), dd.Hour24, dd.Minute, dd.Second, dd.Fraction);
                    v = d.toDateString() + " " + d.toTimeString();
                }
    ...
    this._GetLastEditorClientID = function()
    {
        ...
        {
            var ServerID = colXml.CustomEditorServerID.replace(/\$/ig, "_");
            return ServerID;
        }
...
function WebInputNET_OnDirty(gridName, inputName)
{
    var grid = ISGetObject(gridName);
    var input = ISGetObject(inputName);

    // 11/9/2013: Do not proceed if the WebInput is not actually visible
    if (input.FrameObj.style.display == "none")
        return;

    grid.MarkEdit();
    input.IsDirty = true;
    input.HtmlObj.isDirty = true;
    input.CustomEditor.OnDirty();
}