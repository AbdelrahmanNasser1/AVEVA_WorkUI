<%@ Page Language="C#" AutoEventWireup="true"  Inherits="Workflow.NET.CodeBehind.Bam.SkeltaTemplates_Default_BAM_Analyze_Executing" Theme="" StylesheetTheme="" %> 
<%@ Register Assembly="ISNet.WebUI.WebGrid" Namespace="ISNet.WebUI.WebGrid" TagPrefix="ISWebGrid" %>
<%@ Register TagPrefix="telerik" Namespace="Telerik.Web.UI" Assembly="Telerik.Web.UI" %>
<!DOCTYPE HTML>
<html>
<head id="Head1" runat="server">
    <title>Executing Workflows Page</title>   
...
function ShowProcessView(gridId,tblName,rowIndex,cellIndex,cellEl)
{
    ...
    //var bamWin=window.open(Urls, 'ProcessValue', 'height=500,width=700,resizable=yes,scrollbars=no,status=no,top='+ top +',left='+left);
...
function ShowPreviewText(gridId,tblName,rowIndex,cellIndex,cellEl)
{
    var grid = wgGetGridById("WebGrid1");
    if (grid.GetSelectedObject() == null)
    {
    ...
    if(cell==null)
    return;   
    var Comment =cell.getAttribute("skcommentVal"); 
    if(Comment  == null)
      {
        return;
      }
      {
          //alert(Comment);
          se.ui.messageDialog.showError("<%=GR.GetString("SR_BAM_Desc") %>", Comment);
...
{  
    ...
    //window.parent._DivTree.location.reload();     
    //window.parent.top.relocation.reload();
}
...
function resizeGridHeight() {
    var height = document.getElementById('reportBAMHeight');
    if (document.referrer.indexOf("BAM.aspx") > -1) {
    ...
    <form id="form1" runat="server" style="height:100%" >
       <telerik:RadScriptManager runat="server" ID="RadScriptManager1" />
        ...
        <table style="width:100%;border-collapse: separate; border-spacing: 2px;height:5%;">
            <tr>
                <td style="height:5%;width:90%" class="subtitle">
                    &nbsp;<%:DisplayString%>
                ...
                <td style="height:5%;width:2%;vertical-align:middle;">
                    <img src="<%=cssPath %>BAM/images/expand.gif" style="cursor: hand;vertical-align:middle" onclick="ExpandAll()"
                        title="<%=_ExpandAllToolTip%>"  />
                ...
                <%}
                  ...
                  { %>
                <td style="text-align:right;vertical-align:middle;width:2%;height:5%;padding-right:5px;">
                    <img src="<%=cssPath %>BAM/images/save-layout.gif" style="cursor: hand;hspace:10;vertical-align:middle"
                        onclick="SaveLayout()" title="<%=_ImageToolTip%>"  />
                    <input type="hidden" id="SaveLayOutId" name="SaveLayOut" />
                    <asp:Button OnClick="_IdSaveBtn_Click" Style="display: none" CssClass="input-buttonwork"
        ...
        <div id="reportBAMHeight" style="width:100%;height:85%; overflow:hidden;">
            <ISWebGrid:WebGrid ID="WebGrid1" runat="server" Height="100%" Width="99.8%" HorizontalAlign="NotSet"
                        AllowAutoDataCaching="false" DataCacheStorage="PageCache" OnPrepareExportExecute="WebGrid1_PrepareExportExecute"
