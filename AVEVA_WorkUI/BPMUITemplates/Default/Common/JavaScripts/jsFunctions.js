...
function validateDate(MonValue, DayValue, YearValue) {
    ...
    if (MonValue == 2 && (YearValue % 4) != 0) { //if feb and day selected is greater than 28 (if the year selected is not a leap year)
        if (DayValue > 28) {
            return false;
        }
    }
...
function RefreshWorkItemList() {
    try {
        if (document.referrer.indexOf('type=mail') > 0) {
            //this functionliaty is if usercoms to workitem detail page from EMAIL then we should not close the window and give alert message.

            //after alert message,documnet view frame is showing blank page but manage workitem page still displays so hide the div tags for subject and notes
            ...
            if (typeof (self.opener) != 'undefined' && self.opener != null) {

                self.opener.RefreshGridfromdetwindow();
                window.close();
            }
            else if (window.parent.GetRadWindow != 'undefined' && window.parent.GetRadWindow != null) {
                window.parent.RefreshGridfromdetwindow();
                window.parent.closeRadWindow();
            }
            else {
                if (self.opener)
                {
                ...
                {
                    if (top.window.RefreshGridfromdetwindow)
                    {
                        top.window.RefreshGridfromdetwindow();
                    }
                    else
                    {
                        window.parent.opener.window.RefreshGridfromdetwindow();
                        top.window.close();
                    }
                }
                //this change is to add the when the online forms is loading as modal window the modal window was not getting closed
                //because window.parent.GetRadWindow is undefined, So when the self.opener and window.parent.GetRadWindow was undefined and topwindow closeRadWindow exists
                //Close the Rad window.
                if (!self.opener && !window.parent.GetRadWindow && top.window.closeRadWindow)
                {
                    top.window.closeRadWindow();
                }
            }
        }
    }
...
function RefreshWorkItemListForNextWorkItem() {
...
function RefreshForGroupUI() {
    window.parent.RefreshGridfromdetwindowforGroupUI();

}

function CloseForGroupUI() {
    window.parent.ClosewindowforGroupUI();
}
...
function loadcontent(src, pageview) {
    if (pageview == "Seperate") {
        ...
        window.open(src + "&DisplayType=SEPERATEVIEW", "ActivityView", "height=375,width=780,resizable=yes");
    }
...
//For time display, set the DisplayType as 0 and for Date and time it's 1
function WriteMobileClientTime(TimeVal, DisplayType, TimeFormat) {
    ...
    if (TimeFormat == "24HR") {
        ...
        if (seconds <= 9)
            seconds = "0" + seconds
        var assignedtime = hours + ":" + minutes + ":" + seconds;
    }
    else {
        var dn = "AM"
        if (hours >= 12) {
    ...
    if (DisplayType == 0) {
        ...
        // If the year is same, it won't be displayed.
        if (TodaysDate.getFullYear() == writeTime.getFullYear()) {
            document.write(GetMonthName(writeTime.getMonth() + 1) + "-" + writeTime.getDate() + ", " + assignedtime)
        }
        else {
...
function openWorkItemUINewWindow(openWin, workItemId, application, workflow, channel, form, encryptedQueryString) {
    var myframe;
    var left = (screen.width / 2) - (700 / 2);
    var top = (screen.availHeight / 2) - (500 / 2);    
    if (openWin == 1) {
        window.open(encryptedQueryString + '&displaytype=reassignworkitem', "ReassignActivity", "height=500,width=700,resizable=yes,top=" + top + ", left=" + left);
    }
    if (openWin == 2)
    {     
        window.open(encryptedQueryString + '&displaytype=resourcelist', "ResourcesList", "height=500,width=700,resizable=yes,top=" + top + ", left=" + left);
    }
...
function selected(oRow, mycol) { oRow.className = 'GridSelectedRowStyle'; } //There is no such class named N-ListBGCOLORCOLSELECTED
function deselected(oRow, mycol) { oRow.className = mycol; }
function ExpandCollapseDiv(imgObj, divName) {
    var imageName = imgObj.src.substring(imgObj.src.lastIndexOf('/') + 1);
    var imagePath = imgObj.src.substring(0, imgObj.src.lastIndexOf('/') + 1);
    if (imageName == 'btnPlus.gif') {
        document.getElementById(divName).style.display = 'block';
        imgObj.src = imagePath + 'btnMinus.gif';

    }
    if (imageName == 'btnMinus.gif') {
        document.getElementById(divName).style.display = 'none';
        imgObj.src = imagePath + 'btnPlus.gif';
    }
}
