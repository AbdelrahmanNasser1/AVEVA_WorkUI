<%@ Page Language="C#" AutoEventWireup="true" Inherits="Skelta.Repository.Web.CodeBehind.VersionGrid" %>
...
<body style="margin:0;height:99%;width:99.7%;overflow:hidden" >
        ...
        return true;
                ...
                <ISWebGrid:WebGrid ID="WebGrid1" runat="server" HorizontalAlign="NotSet" Width="100%" Height="99%" RenderingMode="HTML5"
                        ...
                        <LayoutSettings AlternatingColors="true" CellPaddingDefault="2" GridLineColor="224, 224, 224"
                            ...
                            <ClientSideEvents  OnInitialize="OnInitialize" OnRowContextMenu="ShowContextMenu" OnCellDblClick="DoCellClick" OnRowSelect="RowSelect" onAfterResponseProcess="ResizeGrid1" OnColumnSort="OnWebgridColumnSort"></ClientSideEvents>
    ...
    <script language="javascript">        
        var bellyBarTitle = '<%=_resmgr.GlobalResourceSet.GetString("List_item_Operation_for_list") %>';
         ...
         if (noRowSelected == undefined || noRowSelected == null)
            ...
            var belMulResumePublishHead = '<%=_resmgr.GlobalResourceSet.GetString("ec_Workflow_MultiResumePublishHead") %>';
            var belMulResumePublishMessage = '<%=_resmgr.GlobalResourceSet.GetString("ec_Workflow_MultiResumePublishMessage") %>';
            var belMulPauseUnPublishHead = '<%=_resmgr.GlobalResourceSet.GetString("ec_Workflow_MultiPauseUnPublishHead") %>';
            var belMulPauseUnPublishMessage = '<%=_resmgr.GlobalResourceSet.GetString("ec_Workflow_MultiPauseUnpublishMessage") %>';
            var belMulPublishStatusHead = '<%=_resmgr.GlobalResourceSet.GetString("ec_Workflow_PublishStatusHead") %>';
            var belMulPublishStatusMessage = '<%=_resmgr.GlobalResourceSet.GetString("ec_Workflow_PublishStatusMessage") %>';
            var belMulPublishFailedStatusMessage = '<%=_resmgr.GlobalResourceSet.GetString("ec_Workflow_PublishFailedStatusMessage") %>';
            var belMulUnPublishStatusHead = '<%=_resmgr.GlobalResourceSet.GetString("ec_Workflow_UnPublishStatusHead") %>';
            var belMulUnPublishStatusMessage = '<%=_resmgr.GlobalResourceSet.GetString("ec_Workflow_UnPublishStatusMessage") %>';
            var belMulUnPublishFailedStatusMessage = '<%=_resmgr.GlobalResourceSet.GetString("ec_Workflow_UnPublishFailedStatusMessage") %>';    
    </script>
</body>
</html>
