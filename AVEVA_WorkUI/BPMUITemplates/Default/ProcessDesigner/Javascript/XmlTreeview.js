...
CmbjsTree.prototype.createRoot = function(strIcon, strText, strURL, strTarget,strType,xPath) {
    ...
    //create a new node 
    this.root = new CmbjsTreeNode(strIcon, strText, strURL, strTarget,strType,null,"",false,xPath);
...
//  (nothing)
//-----------------------------------------------------------------
CmbjsTree.prototype.buildDOM = function(divName) {

    //call method to add root to document, which will recursively
    //add all other nodes 
    maxImageIncluded=false;
    this.root.addToDOM(document.getElementById(divName));
}


CmbjsTree.prototype.AssignObject = function(obj) {
...
function CmbjsTreeNode(strIcon, strText, strURL, strTarget,strType,strParent,nodeDataType,IsRepeating,xpath) {
...
CmbjsTreeNode.prototype.addChild = function (strIcon, strText, strURL, strTarget,strType,strParent,nodeDataType,IsRepeating,xpath) {

    // create a new node (NCZ, 1/27/02)
	// var objNode = new CmbjsTreeNode(strIcon, strText, strURL, strTarget);
	//alert("in addChild"+ strParent);
	var objNode = new CmbjsTreeNode(strIcon, strText, strURL, strTarget,strType,strParent,nodeDataType,IsRepeating,xpath);
...
CmbjsTreeNode.prototype.addToDOM = function (objDOMParent) {
   ...
   if(!maxImageIncluded)
    ...
    //tooltip = this.xpath;
    var tooltip = replacestringChars(this.xpath,'$','.')
    
    if(tooltip.substring(0,2)=="..")
        tooltip =tooltip.substring(2,tooltip.length);
    
    if((this.childNodes.length==0)&&(this.id!="root"))
    {
      d.write("<td nowrap=\"nowrap\" class=\"contenttext\"><img hspace=\"1\" src=\"" + this.icon + "\" border=\"0\" align=\"absmiddle\" /> <a href=\"#\" onclick=\"javascript:LoadTreeview('"+this.DataType+"','"+this.text+"','"+this.xpath+"')\" class=\"description\" title='"+tooltip+"'>"+this.text+"</a></td>");
    }
    else
    {
        if(!maxImageIncluded)
 ...
 d.write("<td nowrap=\"nowrap\" colspan=2 class=\"contenttext\"><img hspace=\"1\" src=\"" + this.icon + "\" border=\"0\" align=\"absmiddle\" />  <a href=\"#\" onclick=\"javascript:LoadTreeview('"+this.DataType+"','"+this.text+"','"+this.xpath+"')\" class=\"description\" title='"+tooltip+"'>"+this.text+"</a></td>");
...
function Maximize(mode,div)
{
    ...
    if(divTreeObj!=null)
    {
        if((maximize)&&(mode))
        {
            ...
            divTreeObj.style.left = 20 + 'px';
            divTreeObj.style.width = winWidth - 50 + 'px';
            divTreeObj.style.height = (parseInt(winHeight) - divtop) - 50 + 'px';
	        ...
	        imgObject.src = iSrc ;
	        Spanoftext.innerHTML = Spanoftext.innerHTML.replace("Maximize", "Minimize"); //innerText is not working in firefox7.0
	        maximize=false;
	    }
	    else
	    {
	      if(mode==true)
	       ...
	       if((mode==false)&&(maximize==false))
	       {
	            ...
	            divTreeObj.style.height =  olddivHeight;
	            var iSrc = imgObject.src.replace("icon-minimize.png","icon-maximize.png");
	            Spanoftext.innerHTML = Spanoftext.innerHTML.replace("Minimize", "Maximize"); //innerText is not working in firefox7.0
	            imgObject.src = iSrc;
	            maximize=true;
	       }
	       ...
	       if(document.getElementById("Max"+div)!=null)
	       {
	                        ...
	                        Spanoftext.innerHTML=Spanoftext.innerHTML.replace("Minimize","Maximize");//innerText is not working in firefox3.0
