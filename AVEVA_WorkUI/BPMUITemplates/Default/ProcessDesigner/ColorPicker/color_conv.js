// http://www.codeproject.com/jscript/choosecolordialog.asp 
//////////////////////////////////////////////////
...
function fnShowChooseColorDlg(color,param,path){
	...
	if (document.all) { 
		...
		retVal = window.showModalDialog(
				path+'color_dialog.aspx',args,
				'dialogHeight: 400px; dialogWidth: 245px; center: yes; scroll: No; help:  No; resizable: No; status:no;');
		return fnProcessReturnValue(retVal);
	} else if (document.layers) {
	} else if (document.getElementById) { //mozilla
		var winRef;
		winRef = window.open(
				path+'color_dialog.aspx?'+escape(color)+'&'+escape(param),"_blank",
				'toolbar=no, location=no, directories=no, status=no, menubar=no, scrollbars=no, resizable=no, copyhistory=no, width=245, height=400');
	}
}
...
function CaptureElement(elementDivName,elWidth,elHeight,parentLeft,parentTop,e){
	var el = document.getElementById(elementDivName);
	if(el){
		mX = getMouseXPos(e);
		mY = getMouseYPos(e);
		if( 
...
/////////////////////////////////////////////////////////////////////
	...
	if (document.all) { //IE4 and up
		...
		if(parArray.length==2){
			...
			prevColor = unescape(paramsArray[0]);
			g_param = unescape(paramsArray[1]);
		}
	} 
...
/*	if (document.all) { //IE4 and up
	...
	if (val == "OK")
	{
		if (window.opener)
		{
			...
			{
				window.opener.OnChangeColor(fnProcessReturnValue(retVal), g_param);
				//window.opener.setTimeout("OnChangeColor('"+fnProcessReturnValue(retVal)+"','"+escape(g_param)+"')",10);
				//window.opener.OnChangeColor(fnProcessReturnValue(retVal),g_param);
			}
			else
			{
...
function ShowHideCustColors(buttonName1,buttonName2){
	
	//buttonName1 --Define Custom Colors >>
	//buttonName2 --Define Custom Colors << 
	if (document.all) { //IE4 and up
		if(isShownCustColors){
			window.dialogWidth = 245 + 'px';
			document.getElementById('DefineCustomColors').value = buttonName1;//'Define Custom Colors >>';
		}
		else{
			window.dialogWidth = 560 + 'px';//increased window size because of localization issue... window.dialogWidth = 495 + 'px';
			document.getElementById('DefineCustomColors').value = buttonName2;//'Define Custom Colors <<';
		}
	} else if (document.layers) {
	} else if (document.getElementById) { //mozilla
		if(isShownCustColors){
			window.resizeTo(245,400);
			document.getElementById('DefineCustomColors').value = buttonName1;//'Define Custom Colors >>';
		}
		else{
